...
extension CanvasLayer {
    ...
    var texturePixelTransform: Transform2D<TexturePixelCoordinate, LayerPixelCoordinate> {
        Transform2D(matrix: Transform2D.translate(x: 0, y: size.height) * Transform2D.rotate(x: .pi))
    }
}
